...
async fn get_serach_news_html(promt: String, sort_type: SortType, page: u32) -> String {
    let url = format!(
        "https://www.gazeta.uz/ru/search?q={}&sort={}&page={}",
        promt,
        sort_type.to_string(),
        page
    );
...
fn parce_news(html: String) -> Vec<News> {
    ...
    let cursor = scraper::Selector::parse(
        "body>div>div.lenta>div.leftContainer>div.blockSectionNews>div.newsblock-2>div.nblock",
    )
    .unwrap();
    document
        .select(&cursor)
...
pub fn test_parsing_list_of_news() {
    use std::io::Read;
    let mut file = std::fs::File::open("test.html")
        .expect("please, don't delete test.html file from dirrectory");
    ...
    let news = parce_news(html.to_string());
